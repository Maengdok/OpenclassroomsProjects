...
class RevStr(str):
    """
        ...
        dernière lettre, on la parcourt de la dernière à la première
    ...
    """
        ...
        def __iter__(self):
            """
                ...
                dan le sens inverse de celui d'un 'str'
            """

            return ItRevStr(str) # On renvoie l'itérateur créé pour l'occasion

class ItRevStr:
    """
        Un itérateur permettant de parcourir un chaîne de la dernière lettre
    ...
    def __next__(self):
        """
            Cette méthode doit renvoyer l'élément zsuivant dans le parcours,
...
# Générateurs simples : yield

def mon_generateur():
    """
        Notre premier générateur.
        Il va simplement renvoyer 1, 2 et 3
    """
    yield 1
    yield 2
    yield 3
...
>>> mon_generateur()
> '<generator object mon_generateur at 0x00B9C88>'
...
def intervalle(borne_inf, borne_sup):
    """
        Générateur parcourant la série des entiers entrer borne_inf et borne_sup
    """
...
for nombre in generateur:
    if nombre > 17:
        generateur.close() # Interrution de la boucle

# Envoyer des données à un générateur
...
def intervalle(borne_inf, borne_sup):
    """
        ...
        La valeur qu'on lui passe est la nouvelle valeur de borne_inf.
    """

    borne_inf += 1
    while borne_inf < borne_sup:
        valeur_recue = (yield borne_inf)
        if valeur_recue is not None: # Notre générateur à reçu quelque chose
